...
Scriptpad is a notepad like editor to open/edit/save ******* run 
script (JavaScript) files. This sample demonstrates the use of 
javax.script (JSR-223) API ******* JavaScript engine that is bundled 
with JDK 6.

Scriptpad sample demonstrates how to use Javascript to use Java 
classes ******* objects to perform various tasks such as to modify,
customize Swing GUI or to connect to a running application ******* 
monitor it using JMX (Java Management Extensions) API.

* How to run Scriptpad?

Scriptpad can be run with the following comm*******:
...
in the editor ******* run the same with "Tools->Run" menu. 
...
menu). User can specify JMX hostname ******* port. After connecting, 
user can use "monitoring ******* management" script functions defined 
in "mm.js" (see below).

* Scriptpad Sources

com.sun.demo.scriptpad.Main class is the entry point of this
sample. This class creates ScriptEngine ******* evaluates few
...
3. mm.js
 -- Monitoring ******* Management utilities for JavaScript
...
object called "application". This object has 2 fields ******* a method.
...
* Monitoring ******* Management with Scriptpad:
...
(2) Start scriptpad ******* click on "Tools->JMX Connect" menu.
...
For example, it is possible to load ******* run management scripts that
