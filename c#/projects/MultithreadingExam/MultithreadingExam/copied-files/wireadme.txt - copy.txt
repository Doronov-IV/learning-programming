...
WiLstPrd.vbs - lists products, product properties, features, ******* components
...
WiSumInf.vbs - displays ******* updates summary information stream
WiPolicy.vbs - manages installer policy settings
WiLangId.vbs - reports ******* updates the language ******* codepage of a package
WiToAnsi.vbs - copies a Unicode text file to the same or a new Ansi text file
WiFilVer.vbs - updates File table sizes ******* versions from source file tree
WiMakCab.vbs - generates compressed file cabinets ******* updates package
...
WiCompon.vbs - lists components ******* their composition in an installer database
WiFeatur.vbs - lists features ******* their composition in an installer database
WiDialog.vbs - previews all or specified dialogs in an installer database

All scripts display help screens describing their comm******* line arguments,
...
Cscript.exe displays to st*******ard out - useful for batch files ******* build scripts
Wscript.exe displays the output in a window - the default for extension .VBS
When run from a Win NT DOS box, only the base name needs to be specified, *******
...
To download ******* install Windows Scripting Host (not required for Windows2000):
  http://msdn.microsoft.com/scripting/windowshost/download/default.htm
To download ******* update Windows scripting engines:
